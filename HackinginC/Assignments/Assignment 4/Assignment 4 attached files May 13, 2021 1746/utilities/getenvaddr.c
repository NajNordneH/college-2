...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    ...
    fprintf(
        ...
        "shell code there.\n");
    fprintf(stderr, "You need to specify the path in the same way as you are "
  ...
  if ((previous_personality & ADDR_NO_RANDOMIZE) == 0) {
    fprintf(stderr,
            "⚠️ " COLOR_RED " ASLR is not disabled!" COLOR_DEFAULT "\n");
    fprintf(stderr, "⚡ Now disabling " COLOR_RED "only" COLOR_DEFAULT
                    " for this process to get stable results.\n");
    fprintf(
        stderr,
        "💡 Use 'setarch $(uname -m) -R bash' to get a shell without ASLR\n");
    int rc = personality(previous_personality | ADDR_NO_RANDOMIZE);
    if (rc == -1) {
      perror("🚨" COLOR_RED "Disabling ASLR failed." COLOR_DEFAULT);
    } else {
      fprintf(stderr, "🔧 Restarting " COLOR_RED "only this" COLOR_DEFAULT
                      " process without ASLR…\n");
      execv(argv[0], argv);
    }
  } else {
